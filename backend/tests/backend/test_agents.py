...
from apps.interview.agents.base_agent import (
    BaseInterviewAgent,
    InterviewQuestion,
    InterviewAnswer,
)
...
class TestInterviewQuestion:
    def test_creation(self):
        q = InterviewQuestion(
            text="Why this company?", category="hr", difficulty="easy"
        )
...
class TestHRAgent:
    ...
    def test_can_handle_why_want(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Why do you want this role?", category="hr", difficulty="easy"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_strengths(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="What are your strengths?", category="hr", difficulty="easy"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_weaknesses(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Tell me about your weaknesses", category="hr", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_team(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you work in a team?", category="hr", difficulty="easy"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_leadership(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Describe your leadership style", category="hr", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_conflict(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you handle conflict?", category="hr", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_pressure(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Working under pressure?", category="hr", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_deadline(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you meet deadlines?", category="hr", difficulty="easy"
        )
        assert agent.can_handle(q) is True
    ...
    def test_cannot_handle_technical(self, mock_llm, user_profile, company_context):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Explain binary search", category="technical", difficulty="medium"
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_answer_passes_conversation_history(
        self, mock_llm, user_profile, company_context
    ):
        agent = HRAgent(user_profile, company_context, mock_llm)
        agent.update_history("Q1", "A1")
        q = InterviewQuestion(
        ...
        assert call_kwargs[1]["conversation_history"] == [
            {"question": "Q1", "answer": "A1"}
        ]

    @pytest.mark.asyncio
    async def test_generate_answer_passes_question_type(
        self, mock_llm, user_profile, company_context
    ):
        agent = HRAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
...
class TestManagerAgent:
    def test_can_handle_scenario(self, mock_llm, user_profile, company_context):
        ...
        q = InterviewQuestion(
            text="Describe a scenario where your team failed",
            category="managerial",
            difficulty="hard",
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_cross_functional(self, mock_llm, user_profile, company_context):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Working with cross-functional teams",
            category="managerial",
            difficulty="medium",
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_budget(self, mock_llm, user_profile, company_context):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Managing a budget", category="managerial", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_stakeholder(self, mock_llm, user_profile, company_context):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Stakeholder management strategy",
            category="managerial",
            difficulty="medium",
        )
        assert agent.can_handle(q) is True
    ...
    def test_cannot_handle_coding(self, mock_llm, user_profile, company_context):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Write a function to merge two sorted lists",
            category="coding",
            difficulty="medium",
        )
        assert agent.can_handle(q) is False
    ...
    async def test_generate_answer(self, mock_llm, user_profile, company_context):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Describe your management approach",
            category="managerial",
            difficulty="medium",
        )
        answer = await agent.generate_answer(q)
    ...
    @pytest.mark.asyncio
    async def test_generate_answer_passes_correct_type(
        self, mock_llm, user_profile, company_context
    ):
        agent = ManagerAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you handle conflict resolution?",
            category="managerial",
            difficulty="hard",
        )
        await agent.generate_answer(q)
...
class TestTechnicalAgent:
    def test_can_handle_system_design(self, mock_llm, user_profile, company_context):
        ...
        q = InterviewQuestion(
            text="Design a URL shortener", category="technical", difficulty="hard"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_architecture(self, mock_llm, user_profile, company_context):
        agent = TechnicalAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Describe microservices architecture",
            category="technical",
            difficulty="medium",
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_explain(self, mock_llm, user_profile, company_context):
        agent = TechnicalAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Explain how TCP/IP works", category="technical", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_api(self, mock_llm, user_profile, company_context):
        agent = TechnicalAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Design a REST API for user management",
            category="technical",
            difficulty="medium",
        )
        assert agent.can_handle(q) is True
    ...
    def test_cannot_handle_managerial(self, mock_llm, user_profile, company_context):
        agent = TechnicalAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you prioritize sprint work?",
            category="managerial",
            difficulty="medium",
        )
        assert agent.can_handle(q) is False
...
class TestCodingAgent:
    ...
    def test_can_handle_implement(self, mock_llm, user_profile, company_context):
        agent = CodingAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Implement a LRU cache", category="coding", difficulty="hard"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_algo(self, mock_llm, user_profile, company_context):
        agent = CodingAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Solve the two sum problem", category="coding", difficulty="easy"
        )
        assert agent.can_handle(q) is True
    ...
    def test_can_handle_code_review(self, mock_llm, user_profile, company_context):
        agent = CodingAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Review this code for bugs", category="coding", difficulty="medium"
        )
        assert agent.can_handle(q) is True
    ...
    def test_cannot_handle_technical_concept(
        self, mock_llm, user_profile, company_context
    ):
        agent = CodingAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Explain CAP theorem", category="technical", difficulty="hard"
        )
        assert agent.can_handle(q) is False
    ...
    async def test_generate_answer_passes_correct_type(
        ...
        agent = CodingAgent(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Implement binary search", category="coding", difficulty="easy"
        )
        await agent.generate_answer(q)
        call_kwargs = mock_llm.generate_interview_answer.call_args
...
class TestInterviewOrchestrator:
    ...
    def test_route_managerial_question(self, mock_llm, user_profile, company_context):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="How do you handle stakeholder management?",
            category="managerial",
            difficulty="medium",
        )
        agent = orch.route(q)
    ...
    def test_route_technical_question(self, mock_llm, user_profile, company_context):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
            text="Explain distributed systems", category="technical", difficulty="hard"
        )
        agent = orch.route(q)
    ...
    def test_route_returns_none_for_unhandled(
        self, mock_llm, user_profile, company_context
    ):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
    ...
    @pytest.mark.asyncio
    async def test_process_question_returns_none_for_unhandled(
        self, mock_llm, user_profile, company_context
    ):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
    ...
    @pytest.mark.asyncio
    async def test_process_question_updates_history(
        self, mock_llm, user_profile, company_context
    ):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        q = InterviewQuestion(
    ...
    @pytest.mark.asyncio
    async def test_process_multiple_questions(
        self, mock_llm, user_profile, company_context
    ):
        orch = InterviewOrchestrator(user_profile, company_context, mock_llm)
        questions = [
            ...
            InterviewQuestion(
                text="Write a function to sort", category="coding", difficulty="medium"
            ),
        ]
