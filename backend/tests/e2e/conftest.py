...
def mock_voice_service():
    service = MagicMock()
    service.clone_voice = AsyncMock(return_value={
        "clone_id": "clone_test_001",
        "status": "completed",
        "quality_score": 0.92,
    })
    service.synthesize = AsyncMock(return_value=b"mock-audio-bytes")
    service.detect_language = AsyncMock(return_value={"language": "en", "confidence": 0.95})
    return service
...
def mock_interview_service():
    service = MagicMock()
    service.start_interview = AsyncMock(return_value={
        "interview_id": "intv_test_001",
        "status": "started",
        "questions": [],
    })
    service.process_response = AsyncMock(return_value={
        "question_id": "q_001",
        "answer_evaluated": True,
        "score": 0.85,
        "next_question": "Tell me about your experience.",
    })
    service.end_interview = AsyncMock(return_value={
        "interview_id": "intv_test_001",
        "status": "completed",
...
def mock_scoring_service():
    service = MagicMock()
    service.calculate_score = AsyncMock(return_value={
...
def mock_trigger_service():
    service = MagicMock()
    service.detect_triggers = AsyncMock(return_value={
        "triggers": [],
        "risk_level": "low",
    })
    service.activate_proctoring = AsyncMock(return_value={
        "active": True,
        "mode": "enhanced",
    })
    return service
...
def mock_orchestrator_service():
    service = MagicMock()
    service.route_to_agent = AsyncMock(return_value={
        "agent_id": "agent_001",
...
def sample_scoring_payload():
    return {
        ...
        "candidate_id": "cand_001",
        "responses": [
            {"question_id": "q_001", "answer": "I have 5 years of experience", "duration_seconds": 30}
        ],
    }
